...
| skippedIsNotIssue        | Optional   | false     | Delegates skipped-item handling to `@reportportal/client-javascript`. <br/> *true* - the client adds system attribute `skippedIssue: false` to the launch and automatically sets `issueType: NOT_ISSUE` for items finished with `status: SKIPPED` (they won’t appear in **To Investigate**). <br/> *false* - no NOT_ISSUE is applied automatically. |                                                                                                                                                                                  |
